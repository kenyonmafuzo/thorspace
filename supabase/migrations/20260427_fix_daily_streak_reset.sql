-- Migration: Correção do reset da streak após 7 dias
-- Bug: após completar 7 dias, a streak ficava travada em 7 ao invés de reiniciar
-- Fix: quando new_streak > 7, resetar para 1 (início de nova sequência)
...
BEGIN
  ...
  ELSE
    ...
    -- Após completar 7 dias, reinicia para 1 (nova sequência de 7 dias)
...
COMMENT ON FUNCTION claim_daily_xp IS 'Calcula e concede XP de login diário. Ciclo de 7 dias: ao completar o dia 7, reinicia do dia 1 para nova sequência.';
