...
if (typeof window !== "undefined") {
  ...
  document.addEventListener("visibilitychange", async () => {
    ...
    if (hiddenMs < 3000) {
      console.log(`[WAKE] skipping (only hidden ${hiddenMs}ms)`);
      return;
    }

    // Hidden > 10 minutes → silent reload is cleanest (AAA pattern)
    if (hiddenMs > 10 * 60 * 1000) {
      console.log(`[WAKE] hidden ${Math.round(hiddenMs/60000)}min → reloading`);
      window.location.reload();
      return;
    }

    if (_wakeupLock) {
      console.log("[WAKE] recovery already running — skipping");
      return;
    }
    _wakeupLock = true;
    console.log(`[WAKE] wakeup (hidden ${Math.round(hiddenMs/1000)}s) — settling`);
    ...
    console.log("[WAKE] firing thor_wakeup_ready");
