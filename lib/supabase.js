...
if (typeof window !== "undefined") {
  ...
  const _handleWakeup = async (eventType) => {
    ...
    try {
      // getSession() normally reads localStorage without a network call.
      // BUT if the token is expired Supabase will kick off a refresh internally,
      // which can throw AbortError if autoRefreshToken is already running.
      // Wrap it so that path degrades gracefully to "wait for TOKEN_REFRESHED".
      let session = null;
      try {
        const { data } = await supabase.auth.getSession();
        session = data?.session ?? null;
      } catch (sessionErr) {
        // AbortError = autoRefreshToken is already mid-refresh.
        // TOKEN_REFRESHED will fire when it finishes — just wait for it.
        _expectingWakeupRefresh = true;
        console.log("[WAKE] getSession() threw (autoRefresh mid-flight) — waiting for TOKEN_REFRESHED");
