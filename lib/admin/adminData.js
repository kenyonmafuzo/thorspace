...
export async function getUserById(userId) {
  const db = getAdminClient();
  const [{ data: profile, error }, { data: stats }, { data: progress }] = await Promise.all([
    db.from("profiles")
      .select("id, username, avatar_preset, is_vip, vip_expires_at, vip_plan, vip_stripe_session_id, created_at")
      .eq("id", userId)
      .maybeSingle(),
    db.from("player_stats").select("*").eq("user_id", userId).maybeSingle(),
    db.from("player_progress").select("level, xp, total_xp").eq("user_id", userId).maybeSingle(),
  ]);
  if (error) throw error;
  if (!profile) return null;
  return { ...profile, player_stats: stats ?? null, player_progress: progress ?? null };
}
