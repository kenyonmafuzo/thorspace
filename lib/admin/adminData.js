...
export async function getUsers({ page = 1, limit = 30, search = "", filter = "all" } = {}) {
  ...
  let query = db
    .from("profiles")
    .select(
      "id, username, display_name, email, is_vip, vip_expires_at, vip_plan, level, created_at, avatar_url",
      { count: "exact" }
    )
    .order("created_at", { ascending: false })
...
export async function getUserById(userId) {
  ...
  const { data, error } = await db
    .from("profiles")
    .select("*, player_stats(*), player_progress(*)")
    .eq("id", userId)
    .maybeSingle();
  if (error) throw error;
  return data;
...
export async function getVipList({ page = 1, limit = 30, status = "active" } = {}) {
  ...
  const { data, count, error } = await db
    .from("profiles")
    .select("id, username, email, is_vip, vip_expires_at, vip_plan, vip_stripe_session_id, created_at", {
      count: "exact",
    })
