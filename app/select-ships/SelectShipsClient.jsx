  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ margin: "0 0 14px", color: "#00E5FF", fontFamily: "'Orbitron',sans-serif", fontSize: 20, letterSpacing: 1 }}>
          Monte sua Esquadrilha
        </h2>
        <p style={{ margin: "0 0 10px", color: "#ccc", fontSize: 15, lineHeight: 1.7 }}>
        ...
        <p style={{ margin: "0 0 10px", color: "#ccc", fontSize: 15, lineHeight: 1.7 }}>
          ...
          Um menu vai abrir para você escolher o <strong style={{ color: "#FFD700" }}>tipo de tiro</strong> antes de confirmar.
        </p>
        <p style={{ margin: "0 0 28px", color: "#666", fontSize: 13, lineHeight: 1.6 }}>
          Pode repetir a mesma nave nos três slots. Cada slot pode ter um tiro diferente.
        </p>
        <button
          onClick={onClose}
          style={{
            ...
            color: "#00E5FF", fontSize: 16, fontWeight: 700,
...
export default function SelectShipsClient() {
  ...
  // 3 squad slots: null | { shipConfig, shotType }
  const [slots, setSlots] = useState([null, null, null]);
  // Ship being configured right now (on card click)
  const [pendingShip, setPendingShip] = useState(null);
  ...
  const handleShotTypeChange = async (shotType) => {
    if (pendingShip) {
      // Slot flow: drop ship into next empty slot
      const firstEmpty = slots.findIndex(s => s === null);
      if (firstEmpty !== -1) {
        const newSlots = [...slots];
        newSlots[firstEmpty] = { shipConfig: pendingShip, shotType };
        setSlots(newSlots);
        const newPreferences = { ...shotPreferences, [(firstEmpty + 1).toString()]: shotType };
        setShotPreferences(newPreferences);
        if (userId) {
          await supabase.from("profiles").update({ shot_preferences: newPreferences }).eq("id", userId);
        }
      }
      setPendingShip(null);
    }
  ...
  const handleReadyForBattle = async () => {
    ...
    slots.forEach((slot, i) => {
      shotPrefs[(i + 1).toString()] = slot.shotType;
      selectedShipIds.push(slot.shipConfig.id);
    });
    if (typeof window !== "undefined") {
      localStorage.setItem("thor_shot_preferences", JSON.stringify(shotPrefs));
      localStorage.setItem("thor_selected_ships", JSON.stringify(selectedShipIds));
    }
    if (userId) {
      await supabase.from("profiles").update({ shot_preferences: shotPrefs }).eq("id", userId);
    }
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={containerStyle}>
        ...
        <div style={carouselWrapStyle}>
          ...
          {(() => {
            ...
            return (
              <div
                style={{ ...shipCardStyle, opacity: isLocked ? 0.75 : 1, cursor: (!isLocked && filledCount < 3) ? "pointer" : "default" }}
                onClick={() => {
              ...
              >
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 14 }}>
                  {SHIPS_CONFIG.map((_, i) => (
                    <button
                      key={i}
                      onClick={(e) => { e.stopPropagation(); setSelectedShipIdx(i); }}
                      style={{
        ...
        <div style={{ display: "flex", gap: 14, justifyContent: "center", marginBottom: 24, flexWrap: "wrap" }}>
          {slots.map((slot, idx) => (
            <div
              key={idx}
              style={{
                width: 110, minHeight: 130, borderRadius: 14,
                border: slot ? "1px solid rgba(0,229,255,0.5)" : "1px solid rgba(0,229,255,0.15)",
            ...
            >
              <div style={{ fontSize: 10, color: "#555", fontFamily: "'Orbitron',sans-serif", marginBottom: 6, letterSpacing: 1 }}>
                SLOT {idx + 1}
              </div>
              {slot ? (
                <>
                  <img
                    ...
                    style={{ width: 50, height: 50, objectFit: "contain", filter: "drop-shadow(0 0 6px #00E5FF55)" }}
                    onError={e => { e.currentTarget.style.opacity = "0.3"; }}
                  />
                  <div style={{ color: "#00E5FF", fontWeight: 700, fontSize: 11, fontFamily: "'Orbitron',sans-serif", marginTop: 6, textAlign: "center" }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "50%", background: "rgba(255,50,50,0.15)",
                      border: "1px solid rgba(255,50,50,0.4)", color: "#FF5555",
                ...
                <div style={{ color: "rgba(0,229,255,0.2)", fontSize: 26, marginTop: 6 }}>＋</div>
              )}
            </div>
          ))}
        </div>
        ...
        {allFilled && (
          <button
            ...
            style={{ ...shotTypeButtonStyle, marginBottom: 16 }}
            onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(0,229,255,0.5)'; e.currentTarget.style.borderColor = '#00E5FF'; }}
            onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(0,229,255,0.3)'; e.currentTarget.style.borderColor = 'rgba(0,229,255,0.4)'; }}
          >
      ...
      <ShotTypeModal
        ...
        onClose={() => { setShotTypeModalOpen(false); setPendingShip(null); }}
